...
module.exports.run = async function(client, message, args) {
	if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send({
        ...
        const Current = await db.fetch(`AntiSpamOn_${message.guild.id}`);
        ...
        await db.set(`AntiSpamOn_${message.guild.id}`, Type.toLowerCase() === "on" ? "On" : Upper);
 
        let success = new MessageEmbed()
        .setColor(embedcolor)
        .setTitle(`Success`)
        .setDescription(`Anti Spam System Has Been ${Upper === "On" ? "Enabled" : "Disabled"} - <@${message.author.id}>`)
