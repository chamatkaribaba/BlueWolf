...
module.exports.run = async function(client, message, args) {
    ...
    if (!args[0]) {
      let b = await db.fetch(`goodbye_${message.guild.id}`);
      let channelName = message.guild.channels.cache.get(b);
      if (message.guild.channels.cache.has(b)) {
        return message.channel.send(
          `**goodbye Channel Set In This Server Is \`${channelName.name}\` Channel!**`
        );
    ...
    try {
      let a = await db.fetch(`goodbye_${message.guild.id}`);

      if (a === channel.id) {
        return message.channel.send(
          "**This Channel is Already Set As goodbye Channel**"
        );
      } else {
        client.guilds.cache
          .get(message.guild.id)
          .channels.cache.get(channel.id)
          .send("**goodbye Channel Set!**");
        db.set(`goodbye_${message.guild.id}`, channel.id);

        message.channel.send(
          `**goodbye Channel Has Been Set Successfully in \`${channel.name}\`**`
        );
      }
      return;
    } catch (e) {
