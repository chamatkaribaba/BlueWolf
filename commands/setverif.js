...
module.exports.help = {
    ...
    decreption: "setverification",
    aliases: ["sv"]
}

module.exports.run = async function(client, message, args) {
          let prefix;
        let fetched = await db.fetch(`prefix_guild_${message.guild.id}`) || "=";
  ...
  !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("**You Do Not Have Required Permissions! - [ADMINISTRATOR]!**");
        ...
        try {
            ...
            if (channel.id === a && role.id === b) {
                ...
                client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send(`**Welcome To ${message.guild.name}!\nTo Get Verified Type - \`${prefix}verify\`**`);
