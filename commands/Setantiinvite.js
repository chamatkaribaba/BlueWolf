...
if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send({
        ...
        const Current = await db.fetch(`AntiInviteOn_${message.guild.id}`);
        ...
        await db.set(`AntiInviteOn_${message.guild.id}`, Type.toLowerCase() === "on" ? "On" : Upper);
 
        let success = new MessageEmbed()
        .setColor(embedcolor)
        .setTitle(`Success`)
        .setDescription(`Anti Invite System Has Been ${Upper === "On" ? "Enabled" : "Disabled"} - <@${message.author.id}>`)
