...
module.exports.help = {
    name: "setmessagelogschannel",
    decreption: "setmessagelogschannel",
    aliases: ["ml"]
}
...
module.exports.run = async function(client, message, args) {
    ...
    if (!args[0]) {
      let ghost = await db.fetch(`messagelogs_${message.guild.id}`);
      let channelName = message.guild.channels.cache.get(ghost);
      if (message.guild.channels.cache.has(ghost)) {
        return message.channel.send(
          `**messagelogs Channel Set In This Server Is \`${channelName.name}\` Channel!**`
        );
    ...
    try {
      let a = await db.fetch(`messagelogs_${message.guild.id}`);

      if (a === channel.id) {
        return message.channel.send(
          "**This Channel is Already Set As messagelogs Channel**"
        );
      } else {
        client.guilds.cache
          .get(message.guild.id)
          .channels.cache.get(channel.id)
          .send("**messagelogs Channel Set!**");
        db.set(`messagelogs_${message.guild.id}`, channel.id);

        message.channel.send(
          `**messagelogs Channel Has Been Set Successfully in \`${channel.name}\`**`
        );
      }
      return;
    } catch (e) {
