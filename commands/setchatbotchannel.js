...
module.exports.run = async function(client, message, args) {
    ...
    if (!args[0]) {
      ...
      if (message.guild.channels.cache.has(b)) {
        return message.channel.send(
          `**chatbot Channel Set In This Server Is \`${channelName.name}\` Channel!**`
        );
    ...
    try {
      ...
      if (a === channel.id) {
        return message.channel.send(
          "**This Channel is Already Set As chatbot Channel**"
        );
      } else {
        client.guilds.cache
          .get(message.guild.id)
          .channels.cache.get(channel.id)
          .send("**chatbot Channel Set!**");
        db.set(`chatbot_${message.guild.id}`, channel.id);

        message.channel.send(
          `**chatbot Channel Has Been Set Successfully in \`${channel.name}\`**`
        );
      }
      return;
    } catch (e) {
